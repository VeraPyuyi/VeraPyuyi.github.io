...
const twitterUrl = 'https://x.com/Pyuyi2333';
...
test('localized contact links remain accurate', () => {
  ...
  assert.doesNotMatch(chinese, /https:\/\/x\.com\/Pyuyi233(?:\)|\s|$)/);
  assert.doesNotMatch(english, /https:\/\/x\.com\/Pyuyi233(?:\)|\s|$)/);
  assert.doesNotMatch(siteConfig, /^\s*x_pyuyi233:\s*$/m);
  assert.doesNotMatch(siteConfig, /^\s*url:\s*https:\/\/x\.com\/Pyuyi233\s*$/m);
